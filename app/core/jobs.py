...
from app.core.model_selector import BLOCKED_PROVIDERS
        ...
        try:
            ...
            if not model_permissiveness and model_provider:
                model_permissiveness = (
                    "restricted" if model_provider in BLOCKED_PROVIDERS
                    else "permissive"
                )

            # Compliance gate: reject nodes generated by blocked providers
            if model_provider in BLOCKED_PROVIDERS:
                raise ValueError(
                    f"Flash returned blocked provider '{model_provider}' "
                    f"(text_model={text_model}, image_model={image_model}). "
                    f"Clockchain requires permissive-only models."
                )
